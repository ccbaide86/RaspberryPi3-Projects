...
_start:
    ...
    // Abrir archivo para escribir valor 1
    ldr r0, =file_path
    ...
    ldr r0, =0          // pin = 0
    mov r1, #0          // LOW
    bl digitalWrite     // digitalWrite(pin, LOW)

    // Esperar 1 segundo
    bl delay_1s

    // Abrir archivo para escribir valor 0
    ldr r0, =file_path
    ldr r1, =file_write_mode
    bl fopen
    cmp r0, #0
    ...
    mov r1, r0          // Guardar el puntero del archivo en r1
    ldr r0, =format_string
    mov r2, #0          // Valor a escribir en el archivo
    bl fprintf          // fprintf(pf, "%d", 0)
